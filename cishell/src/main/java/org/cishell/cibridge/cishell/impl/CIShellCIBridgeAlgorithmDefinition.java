...
public class CIShellCIBridgeAlgorithmDefinition extends AlgorithmDefinition {
    ...
    private static final HashSet<String> GENERAL_PROPERTIES = Stream.of(
            IN_DATA, OUT_DATA, LABEL, DESCRIPTION, ALGORITHM_TYPE, REMOTEABLE, MENU_PATH, CONVERSION, AUTHORS, //standard algorithm properties
            "author", IMPLEMENTERS, INTEGRATORS, DOCUMENTATION_URL, REFERENCE, REFERENCE_URL, WRITTEN_IN, //standard algorithm properties
            SERVICE_PID, SERVICE_ID, COMPONENT_ID, COMPONENT_NAME, OBJECTCLASS, SERVICE_BUNDLEID, SERVICE_SCOPE //other service related properties
    ).collect(Collectors.toCollection(HashSet::new));

    public CIShellCIBridgeAlgorithmDefinition(ServiceReference<AlgorithmFactory> reference, AlgorithmFactory algorithmFactory) {
        ...
        if (reference.getProperty(IN_DATA) != null) {
            Arrays.stream(reference.getProperty(IN_DATA).toString().split(",")).sequential().map(String::trim).forEachOrdered(getInData()::add);
        } else {
            //todo should we set the default value of a singleton list with a null value??
            getInData().add("null");
        }
        if (reference.getProperty(OUT_DATA) != null) {
            Arrays.stream(reference.getProperty(OUT_DATA).toString().split(",")).sequential().map(String::trim).forEachOrdered(getOutData()::add);
        } else {
            //todo should we set the default value of a singleton list with a null value??
            getOutData().add("null");
        }
        ...
        Arrays.stream(reference.getPropertyKeys())
                .filter(propertyKey -> !GENERAL_PROPERTIES.contains(propertyKey))
                .forEach(propertyKey -> getOtherProperties().add(new Property(propertyKey, reference.getProperty(propertyKey).toString())));
    }
    ...
    public String toString() {
        return "CIShellCIBridgeAlgorithmDefinition{" +
